...
export default function AdminStatData({ refreshTrigger }) {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/student/all')
      .then(res => {
        ...
        const deactivatedTutors = allUsers.filter(u =>
          u.role?.toLowerCase() === 'tutor' && !u.is_active
        ).length;
        ...
        const deactivatedStudents = allUsers.filter(u =>
          u.role?.toLowerCase() === 'tutee' && !u.is_active
        ).length;
  ...
  const stats = [
    ...
    { label: 'Deactivated Tutors', value: deactivatedTutors, icon: <PersonOffIcon />, color: 'red' },
    { label: 'Active Students', value: activeStudents, icon: <SchoolIcon />, color: 'purple' },
    { label: 'Deactivated Students', value: deactivatedStudents, icon: <PersonOffIcon />, color: 'yellow' },
  ];
